...
* **Admin/GM self-carve-out (2026-09-14).** Admin/General Manager have no real SBU membership — same placeholder-`sbu_id` situation as BR-OP-12 — so they structurally fail this rule for every candidate, including themselves. Surfaced live: Haroon (General Manager) added a Sales Staff member to a deal's split at 20%, then couldn't find his own name to take the remaining 80%. **Narrowly fixed, not by giving Admin/GM a real SBU** (rejected — see BR-CAT-01/BR-OP-12's own "no fake SBU" history): an Admin/GM caller may now add *only themselves* — not a blanket exemption for any Admin/GM landing in the submitted list, and a normal role adding themselves cross-SBU is still rejected exactly as before. Enforced in both places: the picker (`organization/repository.py`'s `scope="sbu"` branch) and the save-time check (`replace_splits`, now taking `role_name` to know whether the caller qualifies).
* **New participant awareness notification (2026-09-14).** Whoever's genuinely new to the split (the same set BR-FIN-06 itself validates, not a re-save of an existing participant's percentage) gets a non-urgent, in-app notification (`SPLIT_ADDED`) — except when someone adds themselves, which needs no notification. Same posture as BR-OP-14's `GATE_OVERRIDE_NAMED`: awareness only, the split already exists by the time it fires, nothing waits on the recipient.
...
### BR-ACC-03: Near-Duplicate Hospital Warning (2026-08-30) — Option B decided 2026-09-07
* **Status:** built to validate the hypothesis in `docs/Duplicate-Hospital-Decision-Brief-2026-08-29.md`'s Option B, after two real incidents (see that brief's log) each cost a rep a real, already-logged deal. **Haroon decided Option B on 2026-09-07** — approved for rollout as-is (soft-block warning, not an Admin-only create restriction).
* **Rule:** creating a new Account checks for existing Accounts with a similar name *anywhere in the same operational zone branch* (e.g. all of "North Kerala," not just whichever district/taluk zone was picked — an exact `zone_id` match would miss a hospital filed at a parent zone against the same hospital re-filed at a child zone). If a similar Account exists, the create is soft-blocked with the matching candidate(s) returned; the caller may resubmit with `force_create=true` to proceed anyway.
* **Zone-branch boundary is `zone_level="ZONE"`, not "no parent at all."** Found 2026-08-30, same day as the rule was first tuned: once Kerala/Karnataka were added as `zone_level="STATE"` parents above North Kerala/South Kerala/Bangalore/etc., walking to the topmost ancestor merged all of Kerala (or Karnataka) into one pool — e.g. a Kozhikode (North Kerala) hospital getting compared against an Ernakulam (South Kerala) one. Fixed by stopping at the nearest `zone_level="ZONE"` ancestor instead.
* **Short-query gap, found and fixed same day (2026-08-30):** a bare "Aster" failed to warn against any of several real Aster-branded hospitals in the same zone, because the scorer divided by the *longer* name's word count. Fixed by making the score asymmetric (`score_query_containment`): it now measures how much of the *typed* name is covered by an existing name, not a symmetric similarity — a short query counts as a full match once every word in it is found, regardless of how many extra words the candidate has. Re-validated against every known true/false-positive pair before shipping (all held, including "Medical Trust Hospital" vs "Trust well Hospital," which still correctly scores 0 since "Trust" is itself a stopword).
* **Similarity scoring:** generic hospital-industry words ("Hospital," "Medical College," "Diagnostics," etc.) are stripped before comparing, then the remaining significant words are fuzzy-matched one-to-one (`account/duplicate_matching.py`). Raw name similarity was tried first and rejected — on real directory data, a false-positive pair (two unrelated real hospitals sharing generic wording) scored *higher* than a true duplicate pair, so no single raw-similarity threshold could separate the two.
* **Known trade-off:** two real, differently-named branches of the same hospital *brand* in different towns can still cross the threshold (e.g. two "EMS Cooperative Hospital" branches, only the town name differing) — accepted because the UX is a one-tap-dismiss warning, not a hard block, and erring toward catching more real duplicates costs an occasional easy false alarm rather than a missed one.
...
* **Why:** SBU (Imaging vs. Critical Care) is a hard RLS security boundary everywhere else in the app. The Area Manager RLS branch's `manager_id` condition (folded in from the retired Sales Manager tier, migration `0021` — see ADR-009's amendment) grants visibility over "opportunities owned by people whose `manager_id` = me" *alongside* the same branch's own `sbu_id = cabio_app_sbu_id()` check — that outer SBU check is only a real boundary if `manager_id` itself is always assigned within the same SBU to begin with. Without this check, an Admin/GM could (by mistake or otherwise) assign a Sales Staff person's manager to an Area Manager in the *other* SBU, letting that manager see opportunities across the boundary.
