...
### BR-ACC-03: Near-Duplicate Hospital Warning (2026-08-30) — prototype, pending Haroon's decision
* **Status:** built to validate the hypothesis in `docs/Duplicate-Hospital-Decision-Brief-2026-08-29.md`'s Option B, after two real incidents (see that brief's log) each cost a rep a real, already-logged deal. **Not yet approved for general rollout** — the brief's decision (Option A vs. B) is still Haroon's to make.
