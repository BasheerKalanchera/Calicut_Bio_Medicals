...
- **Supavisor pooler mode — `DATABASE_URL` must use the transaction-mode port (`6543`), not session-mode (`5432`).** Session mode's client ceiling is small and fixed (15 on Supabase's free tier) and is held for a client's *entire* session; ordinary app traffic exhausted it in practice (UAT outage, 2026-09-11 — root-caused to a Supabase platform incident leaving the pooler's session slots stuck, made much worse by `DB_POOL_SIZE`+`DB_MAX_OVERFLOW` alone requesting up to 30 connections against a 15-slot cap; see `docs/Progress-Archive-2026-09.md`'s 2026-09-11 entry for the full incident). Safe because `set_rls_context()` (§9) sets RLS context via `set_config(..., true)` — transaction-scoped, not session-scoped. `ADMIN_DATABASE_URL` stays on the session-mode port `5432` — Alembic migrations and `pg_dump` (`scripts/backup_uat.ps1`) are not guaranteed safe under transaction-mode pooling, and this connection's traffic is far too low-volume to ever hit the session-mode ceiling anyway.
