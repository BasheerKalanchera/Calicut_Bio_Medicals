...
the new junior Sales Manager are the two genuinely new levels, added for future growth, currently
unpopulated.

**Directive:** build the data model and RLS for the full 6-tier structure now, not a 4-tier
version extended later — the two new tiers must be first-class from day one even though they
start empty, so today's reality and the future structure are the same code path.
...
## 5. Opportunity SBU Attribution — Already Decided, Already Built
...
This is airtight given the modeling convention in §8 below — an Opportunity's SBU is never
ambiguous by construction.

---

## 6. SBU Transfers — Frozen Attribution + Manual Ownership Handoff
...
- **Frozen `sbu_id`** (§5, already built) protects *visibility and reporting integrity* — a deal
...
- **Manual handoff** (chosen approach — Decision, §14) is the human decision about *who actively
...
## 7. Cross-SBU Splits — Documented as a Future Capability, Not in Use Today
...
capability is built. Cheap, low-risk, prevents the invariant in §5/§8 from quietly breaking.

---

## 8. Project → Opportunity Creation Flow, for Multi-SBU Customer Needs
...
2. She creates her own `Opportunity` under it (`project_id` = the Project, `owner_id` = herself)
   — `sbu_id` stamped as Imaging at that moment (§5).
...
answer, so the stamp in §5 never has ambiguity to resolve.

**Companion guard:** block reassigning an Opportunity's owner to someone from a **different** SBU
than its stamped `sbu_id` — a handoff (§6) should only ever go to someone in the same SBU as the
deal.

---

## 9. Enforcement Mechanism — PostgreSQL RLS, Not Application-Layer Filtering
...
**Why this specifically matters here:** the Project's own "Opportunities" tab (§8) is a separate
...
## 10. Document Consistency Review
...
## 11. Current Build Status of Phase 2E RLS: 0% Built
...
## 13. Residual — Not Yet Confirmed by Cabio Leadership
...
| 3 | `opportunity.sbu_id` frozen at creation | Already decided (ADR-035) and already built (§5) |
| 4 | SBU transfers: frozen `sbu_id` + manual ownership handoff, combined | Manual handoff alone leaks visibility during the transition window (§6) |
| 5 | Cross-SBU splits: guard added now, capability deferred | Not in use today; prevents future capability from being silently pre-empted (§7) |
| 6 | Multi-SBU customer need = one Project, one Opportunity per SBU | Matches existing Project/Opportunity modeling convention (§8) |
| 7 | Enforcement via PostgreSQL RLS, not per-screen filtering | Already approved direction; closes the Project-tab leak class of bug by construction (§9) |
| 8 | RLS must land and be proven on UAT before Prod go-live | Already decided 2026-07-14; reinforced by this design (§12) |
...
2E build (schema migration, RLS context propagation, restricted role + policies, testing strategy,
the two doc fixes, ADR-009 rewrite) as an explicit, standalone estimate — before it gets silently
assumed to fit inside whatever timeline Milestone 2 or the pilot rollout currently has.
