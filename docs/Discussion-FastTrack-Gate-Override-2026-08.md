...
**Prepared:** 2026-08-20.
**Status:** OPEN — pending decision. Not yet built.
...
| Who can invoke | Any rep, no approval | **Open question — see §5.** REPEAT_ORDER's "any rep, no approval" design is safe *because* it's gated on a structural fact (lead source), not a judgment call. This case is a judgment call, which is exactly what the original paper's §3 flagged as the risk of an ungated skip. |
...
- `gate_override_approver_id` — FK to `user_profile`, constrained to a qualifying role
  (see §5.1).
- `gate_override_reason` — text, mandatory whenever `gate_override_approver_id` is set.
...
### 5.1 Who qualifies as approver?

Since the Sales Manager Tier Collapse, there is no first-line manager role between Sales
Executive and General Manager — the only roles in the system are `Sales Executive`,
`General Manager`, and `Admin`. So "manager approval" here can only mean **General
Manager or Admin**, not a closer first-line manager. Worth confirming that's acceptable
given GM is likely not close to every individual deal the way a first-line manager would
be in a system that had one.

### 5.2 Does the approver need to be the rep's *own* reporting line, or any qualifying role?

The system doesn't currently model a rep→manager reporting hierarchy (zone assignment
isn't a management chain). Simplest option: any user holding General Manager or Admin
role qualifies, no org-hierarchy check. Recommend this default unless there's a reason
to build hierarchy validation just for this.

### 5.3 Reason: free text or a master-data reason list?

`BR-OP-02`/`BR-OP-03` both use master-data reason codes (`hold_reason_id`,
`loss_reason_id`) rather than free text, for consistent reporting. Recommend the same
pattern here — a small `GateOverrideReason` master list (e.g. "Customer declined demo,"
"Deal closed outside normal process, entered after the fact," "Other — see notes") plus
an optional free-text note, rather than pure free text.

### 5.4 Does this need a volume/overuse safeguard from day one, or monitor-then-decide?

Given the expected volume is "occasional," recommend **monitor-then-decide**: ship the
attestation + reporting visibility, revisit only if usage patterns suggest it's becoming
a shortcut rather than a genuine exception — same posture the original paper took toward
REPEAT_ORDER's volume before committing to Option D.

## 6. Recommendation (strawman, for Haroon/Basheer to accept or amend)

- Build the Manager-Attested Gate Override as scoped in §4.
- Approver role: General Manager or Admin (§5.1), no reporting-line check (§5.2).
- Reason: master-data list + optional note (§5.3).
- No overuse safeguard beyond reporting visibility at launch (§5.4).
- Record as `BR-OP-14` once decided, amending `BR-OP-01`'s gate table the same way
  `BR-OP-13` did.
