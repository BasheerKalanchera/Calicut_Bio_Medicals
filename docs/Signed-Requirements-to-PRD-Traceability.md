...
`scripts/generate_scorecard.py` — never hand-edited directly. Edit a
Status or Note here, then re-run the script.
...
**Current tally: 26 Done · 14 Partial · 10 Not started** (50 signed lines
tracked below).
...
| Signed Feature ID | Requirement | PRD Section | Status | Notes | Client Note |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 4.1 | Strict product hierarchy: category → brand → model | 2.1 Product Structure | Partial | SBU → Category → Brand (OEM) → Model are all captured, but Category and Brand are free-text entry — no controlled/enforced pick-list, so inconsistent entries (e.g. "GE" vs "GE Healthcare") aren't prevented | Product categories and brands are captured today as free text rather than a fixed list, so naming can vary slightly between entries. |
