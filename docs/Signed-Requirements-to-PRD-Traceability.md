...
| 2.2 | Pipeline filters by region, product, salesperson | 3.8 Pipeline Management | Partial | Region (Zone) and salesperson (Owner) filters exist on the Pipeline board itself. Product filtering deliberately won't be added to the board — Basheer's decision, 2026-09-14: it belongs on the standalone Pipeline Report (PRD 5.6 Core Reports, scheduled to build) instead of the Kanban filter bar | Region and salesperson filtering are live on the pipeline board today. Product filtering will be available on the upcoming Pipeline Report instead. |
