...
| 5.1 | Account segmentation by size, specialty, revenue potential | 1.2 Account Segmentation | Not started | **Revenue potential confirmed required** at the 2026-09-24 demo (Haroon/Latheef Bhai): delivered as the **Business Potential** rating (High / Medium / Low) plus a Business Potential Notes field, the same field as 1.3 Customer Tiering below. It will also feed hospital-wise target planning (see 6.1 Beat Planning). Size and specialty segmentation are still open (Customer Tiering plan, question 1). Detail: `docs/Progress-Archive-2026-09.md`, entry "2026-09-25 — Demo follow-up: leadership requests recorded" | |
| 5.1 | Tier 1 / Tier 2 dropdown | 1.3 Customer Tiering | Not started | Required — Latheef Bhai, 2026-09-24. **Renamed at the 2026-09-24 demo: "Business Potential" (High / Medium / Low) replaces Tier 1/2/3**, with a Business Potential Notes field recording why each rating was given; payment reliability stays on the existing payer-behaviour field. Customer Tiering — draft plan committed `5b26889` (`docs/Customer-Tiering-Implementation-Plan.md`), remaining open questions (size vs A/B/C/D, per-SBU rating, hospital groups, who sees/sets it) still with Latheef Bhai/Haroon. Detail: `docs/Progress-Archive-2026-09.md`, entry "2026-09-24 — Customer Tiering: draft plan committed" | |
...
| 3.2 | Real-time actual-vs-target via role-specific dashboards | 5.3/5.4/5.5 Salesperson/Manager/GM Dashboard | Partial | Insights Dashboard live with role-based visibility; no GM-specific widgets yet. **Added at the 2026-09-24 demo (Haroon):** each salesperson's actuals tracked against their own hospital-wise planned targets on the Insights Dashboard, rolling up to Zone/SBU/company — depends on hospital-wise target planning (6.1 Beat Planning, `docs/Backlog.md` "Hospital-wise target planning") | Role-based dashboards are live today; a dedicated top-level leadership view is still to be added. |
...
| Signed Feature ID | Requirement | PRD Section | Status | Notes | Client Note |
| :--- | :--- | :--- | :--- | :--- | :--- |
| *(untagged)* | Basic Beat Planning, upgradable to Google Maps tracking in Phase 2 | 6.1 Beat Planning | Not started | This is the same feature as "Coverage Planning" internally — PRD 6.1's ask (hospitals to cover, planned visits, strategic objective, expected revenue) matches `docs/Coverage-Planning-Implementation-Plan.md` field-for-field. Not a separate gap: already fully scoped, all decisions resolved 2026-09-11 (`docs/Backlog.md`, Milestone 2), just not built yet — queued behind Target Planning. **Scope expanded at the 2026-09-24 demo (Haroon/Latheef Bhai):** each person's quarterly target is to be divided among the hospitals in their coverage area according to each hospital's Business Potential (see 1.3), in addition to visit frequency; hospital-wise targets roll up to Zone, SBU and company level, and actuals are tracked against them (dashboard half under 3.2 below). Design C chosen 2026-09-25 (hospital amounts add up to the quarterly target, brand split on top, one approval); the 2026-09-11 design is superseded — `docs/Discussion-Hospital-Wise-Target-Planning-2026-09.md` | |
...
Following items were asked for by Cabio leadership beyond the signed scope and are
confirmed to build; they aren't built yet. Each moves up into "What we built" once
it is. Kept separate from "Pending" below, which holds ideas leadership hasn't
decided on yet.
