...
| Signed Feature ID | Requirement | PRD Section | Status | Notes | Client Note |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 4.1 | Strict product hierarchy: category → brand → model | 2.1 Product Structure | Partial | SBU → Category → Brand (OEM) → Model are all captured, but Category and Brand are free-text entry — no controlled/enforced pick-list, so inconsistent entries (e.g. "GE" vs "GE Healthcare") aren't prevented. **Dependency, updated 2026-09-18:** Haroon has now confirmed the Product Catalog `name`-derivation change (name computed from Brand+Model+Category via a DB-generated column, order Brand→Model→Category) — full implementation plan written: `docs/Product-Catalog-Name-Derivation-Implementation-Plan.md`. Parked, not started, pending Basheer's go-ahead to build (see Backlog.md). Note this fixes `name`-field drift only — it does **not** convert Brand/Category into a controlled pick-list, so this row's core gap (free-text Brand/Category) remains open even once that change ships. When this feature is built, also revisit Product Performance's Brand-grouped report cards, currently left non-clickable for drill-down because Brand has no stable id, only a normalized free-text string (`backend/app/domains/reporting/repository.py:337-343`) — don't miss wiring that up too. | Product categories and brands are captured today as free text rather than a fixed list, so naming can vary slightly between entries. |
