...
**Current tally: 29 Done · 12 Partial · 9 Not started** (50 signed lines
tracked below).
<!-- TALLY:END -->
...
| Signed Feature ID | Requirement | PRD Section | Status | Notes | Client Note |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 4.1 | Strict product hierarchy: category → brand → model | 2.1 Product Structure | Partial | **Built 2026-09-21** (`cd0d8ab`, migrations 0048/0049): Brand, Category, and Model are now real controlled tables, not free text — the Product Catalog's Add/Edit form picks Brand then Model (Category auto-fills from Model), with inline "+ Add new" for Admin/GM. The 59-product catalog was reseeded from Haroon's corrected list. `product.name` is now trigger-computed from Brand+Model+Category, closing the `name`-drift half of this gap too. Product Performance's Brand-grouped cards now group on a real Brand id (`backend/app/domains/reporting/repository.py`) instead of normalized free text, though the pipeline still has no `brand_id` filter to drill into yet (`docs/Backlog.md`). **Still Partial, not Done:** only smoke-tested live on Dev so far — full manual E2E pass (`docs/Product-Catalog-Name-Derivation-Implementation-Plan.md`'s Verification section) is still pending before this row can flip. | Product categories and brands now come from a fixed, admin-managed list instead of free text — inconsistent naming (e.g. "GE" vs "GE Healthcare") is no longer possible. Full testing is still in progress. |
...
| 3.1 | Target splitting by product category, quarterly/annual tracking | 6.5 Product Category Targets | Not started | Quarterly and annual tracking are both live now (Target Management, 6.4, above). Splitting a target by product category is deliberately deferred to Phase 2 (`docs/Target-Planning-Implementation-Plan.md`'s decided scope) — flat revenue-only targets for Phase 1, no longer blocked on a missing dependency, just out of this phase's scope | |
