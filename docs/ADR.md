...
*   **Impact:** `UserRepository.list_active` gains a `scope` parameter (`"scoped"` | `"sbu"` | `"all"`) so the three different "pick a user" UI pickers (Opportunity Owner reassignment, Split participant, Next Action assignee) can each apply their own eligibility rule from one shared endpoint (`GET /users?scope=...`) instead of all sharing one behavior. See BR-FIN-06 (Split Participant Eligibility) and BR-ACT-06 (Next Action Assignee Eligibility) in `Business-Rules.md`. The Split picker's `scope="sbu"` was originally also zone-restricted (interim, not backend-enforced); dropped 2026-08-07 to match BR-FIN-06's actual server-side rule exactly (same SBU, any zone).
...
### ADR-009: Row-Level Security — Tiered Visibility with Participant Carve-Outs
*   **Decision:** Implement PostgreSQL Row-Level Security (RLS) to scope every user's visibility to opportunities (and their joined-back child/context entities) by role tier, not a flat SBU/Manager split: Admin and General Manager see everything unrestricted; SBU Manager sees their whole SBU; Area Manager sees their SBU narrowed to their zone (`account.zone_id`); Sales Manager sees their direct reports' opportunities (`user_profile.manager_id`); Sales Staff see their own. Two participant carve-outs cut across every tier and are permanent once granted: a user with a split on a deal, or an assigned Next Action reminder on it, can see that opportunity regardless of what their tier would otherwise permit.
*   **Status:** Implemented (Phase 2E, migrations `0008`-`0012`, live on dev since 2026-07-27, committed `7d7155d` 2026-07-30). This entry originally described the pre-build design and contained two real inaccuracies, corrected below now that the design is implemented and verified. Full mechanism: `docs/Phase-2E-Security-Architecture.md`. Full tier-by-tier design rationale: `docs/Opportunity-Access-Hierarchy-Technical-Design.md`.
*   **Rationale:** Prevents data clutter and competitive conflicts between SBUs while maintaining a single source of truth for the enterprise — the original motivation, still accurate — extended to also match Cabio's actual management structure (zone-scoped Area Managers, reporting-line-scoped Sales Managers) rather than a flat two-tier SBU/Manager model, and to guarantee that being handed a follow-up task or given commission credit on a deal always grants visibility into it, independent of org-chart position.
*   **Impact:** RLS policies run under a dedicated, least-privilege `cabio_app` Postgres role (not the table owner, which Postgres exempts from RLS). Policies evaluate 4 session variables (`app.current_user_id`/`sbu_id`/`role_id`/`zone_id`, the last conditional) set once per request by `set_rls_context()`, read back via `cabio_app_uid()`/`cabio_app_sbu_id()`/`cabio_app_role_id()`/`cabio_app_zone_id()`/`cabio_app_role_name()` plus two `SECURITY DEFINER` carve-out functions. **Two corrections to this entry's original text:** (1) the role tier this ADR called "Sales Executives" was renamed "Sales Staff" during the build (migration `0008`) — no tier named "Sales Executives" exists; (2) the original mechanism — "the authenticated user's SBU token claims" (i.e. reading JWT claims directly inside RLS policies) — was never built. The actual, deliberately-chosen mechanism reads session-scoped Postgres GUCs (`SET LOCAL`) stamped by the application after JWT validation resolves a `UserProfile`, not JWT claims evaluated in-policy; see `Phase-2E-Security-Architecture.md`'s Database Role Strategy section for why.
*   **Affected Modules:** Database Infrastructure, Auth Context, All Queries, Opportunity Pipeline, Activity/Reminder, Product Catalog.
*   **Amendment (2026-08-15):** Sales Manager retired as a standalone tier (migration `0021`); its `manager_id`-based rule is folded into the Area Manager branch as an additional OR-condition instead. Real field data gathered 2026-08 showed no one in Cabio's org occupied Sales Manager — every rep reports directly to an Area Manager — and the zone hierarchy (self-referencing tree + `zone_closure`, migration `0019`) already lets an Area Manager cover any granularity, from a whole state to a single taluk, making a dedicated "team lead" tier structurally redundant with "regional manager." This does not reduce the leadership-approved intent behind the original decision — regional and team-level coverage are both still fully delivered, now via one flexible mechanism instead of two role rows; `manager_id` remains a safety net for account-zone/reporting-line drift, not the primary visibility mechanism. Full rationale: `docs/Sales-Manager-Tier-Collapse-Implementation-Plan.md`; updated tier design: `docs/Opportunity-Access-Hierarchy-Technical-Design.md`'s 2026-08-15 revision note.
